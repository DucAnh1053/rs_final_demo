...
import rankfm
import pickle
from utils.tools import recommend_cold_start, specialization_names
...
def load_data():
    ...
    user_ids_filled = [user_id for user_id in user_ids if int(user_id) not in [992, 1024, 808, 522, 237, 820, 1180]] # Cold start users
...
def get_predictions(model_name, user_id=None, cold_start=False, characteristics=None):
    if not cold_start:
        ...
        if model_name == 'KNN':
            result, _ = knn_model.recommend(userid=player_ix, user_items=sparse_matrix[player_ix], N=10, filter_already_liked_items=True)
        elif model_name == 'ALS':
            result, _ = als_model.recommend(userid=player_ix, user_items=sparse_matrix[player_ix], N=10, filter_already_liked_items=True)
        else:
            result = fm_model.recommend(users=[player_ix], n_items=10, filter_previous=True, cold_start='nan').loc[player_ix].values
    else:
...
if model_name == "FM":
    player_type = st.sidebar.radio("Chọn loại người chơi", ("Người chơi đã có tương tác", "Cold Start"))
    if player_type == "Người chơi đã có tương tác":
        user_id = st.sidebar.selectbox("Chọn ID người chơi", user_ids_filled)
    else:
        cold_start = True
        characteristics = st.sidebar.selectbox("Chọn đặc điểm người chơi", player_features)
else:
    user_id = st.sidebar.selectbox("Chọn ID người chơi", user_ids_filled)
...
if 'predictions' in st.session_state:
    for question in st.session_state['predictions']:
        with st.expander(f"Câu hỏi {question._id}"):
            ...
            st.write(f"KC: {question.knowledge_id}")
            ...
            selected_answer = st.radio(f"Chọn câu trả lời", question.answer, 
                                       key=f"answer_{question._id}", 
                                       index=question.answer.index(st.session_state[f"selected_answer_{question._id}"]) if st.session_state[f"selected_answer_{question._id}"] else None)
            ...
            if st.button("Xác nhận ", key=f"confirm_{question._id}"):
                if selected_answer == question.correct_answer:
                    ...
                    st.error("Đáp án sai, đáp án đúng là: " + question.correct_answer)
